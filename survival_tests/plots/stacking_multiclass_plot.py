...
def generate_sbs_vbs_change_table():
    ...
    stacking_new = get_dataframe_for_sql_query(
    ...
    stacking_multiclass = get_dataframe_for_sql_query(
        "SELECT scenario_name, AVG(n_par10) as result FROM (SELECT vbs_sbs.scenario_name, vbs_sbs.fold, stacking_multiclass.approach, vbs_sbs.metric, stacking_multiclass.result, ((stacking_multiclass.result - vbs_sbs.oracle_result)/(vbs_sbs.sbs_result -vbs_sbs.oracle_result)) as n_par10,vbs_sbs.oracle_result, vbs_sbs.sbs_result FROM (SELECT oracle_table.scenario_name, oracle_table.fold, oracle_table.metric, oracle_result, sbs_result FROM (SELECT scenario_name, fold, approach, metric, result as oracle_result FROM `vbs_sbs` WHERE approach='oracle') as oracle_table JOIN (SELECT scenario_name, fold, approach, metric, result as sbs_result FROM `vbs_sbs` WHERE approach='sbs') as sbs_table ON oracle_table.scenario_name = sbs_table.scenario_name AND oracle_table.fold=sbs_table.fold AND oracle_table.metric = sbs_table.metric) as vbs_sbs JOIN stacking_multiclass ON vbs_sbs.scenario_name = stacking_multiclass.scenario_name AND vbs_sbs.fold = stacking_multiclass.fold AND vbs_sbs.metric = stacking_multiclass.metric WHERE vbs_sbs.metric='par10') as final WHERE metric='par10' AND NOT scenario_name='CSP-Minizinc-Obj-2016' GROUP BY scenario_name")

    fig, ax = plt.subplots()  # Create a figure containing a single axes.

    #plt.axhline(np.average(stacking.result), color=color1, linestyle='dashed', linewidth=1)
    plt.axhline(np.average(stacking_multiclass.result), color=color2, linestyle='dashed', linewidth=1)
    plt.axhline(np.average(stacking_new.result), color=color3, linestyle='dashed', linewidth=1)

    width = 0.2  # the width of the bars
    ind = np.arange(len(stacking_new.scenario_name))
    #ax.bar(ind - width / 2, stacking.result, width, color=color1, label='Stacking')
    ax.bar(ind + 3 * width / 2, stacking_new.result, width, color=color3, label='Stacking')

    #ind = np.arange(len(stacking_multiclass.scenario_name))
    ax.bar(ind + width / 2, stacking_multiclass.result, width, color=color2, label='Stacking with Multiclass')

    ax.set_xticks(ind + width / 2)
    ax.set_xticklabels(stacking_multiclass.scenario_name)
