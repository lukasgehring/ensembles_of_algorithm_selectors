...
def generate_sbs_vbs_change_table():
    ...
    voting_ranknig_weighting_1234567 = get_dataframe_for_sql_query(
        "SELECT approach, AVG(n_par10) as result FROM (SELECT vbs_sbs.scenario_name, vbs_sbs.fold, voting_ranking.approach, vbs_sbs.metric, voting_ranking.result, ((voting_ranking.result - vbs_sbs.oracle_result)/(vbs_sbs.sbs_result -vbs_sbs.oracle_result)) as n_par10,vbs_sbs.oracle_result, vbs_sbs.sbs_result FROM (SELECT oracle_table.scenario_name, oracle_table.fold, oracle_table.metric, oracle_result, sbs_result FROM (SELECT scenario_name, fold, approach, metric, result as oracle_result FROM `vbs_sbs` WHERE approach='oracle') as oracle_table JOIN (SELECT scenario_name, fold, approach, metric, result as sbs_result FROM `vbs_sbs` WHERE approach='sbs') as sbs_table ON oracle_table.scenario_name = sbs_table.scenario_name AND oracle_table.fold=sbs_table.fold AND oracle_table.metric = sbs_table.metric) as vbs_sbs JOIN voting_ranking ON vbs_sbs.scenario_name = voting_ranking.scenario_name AND vbs_sbs.fold = voting_ranking.fold AND vbs_sbs.metric = voting_ranking.metric WHERE vbs_sbs.metric='par10') as final WHERE metric='par10' AND approach='voting_ranking_weighting_1_2_3_4_5_6_7' AND NOT scenario_name='CSP-Minizinc-Obj-2016' GROUP BY approach  ")
    ...
    voting_ranknig_weighting_24567 = get_dataframe_for_sql_query(
    ...
    voting_ranking_12346 = get_dataframe_for_sql_query(
        "SELECT approach, AVG(n_par10) as result FROM (SELECT vbs_sbs.scenario_name, vbs_sbs.fold, voting_ranking.approach, vbs_sbs.metric, voting_ranking.result, ((voting_ranking.result - vbs_sbs.oracle_result)/(vbs_sbs.sbs_result -vbs_sbs.oracle_result)) as n_par10,vbs_sbs.oracle_result, vbs_sbs.sbs_result FROM (SELECT oracle_table.scenario_name, oracle_table.fold, oracle_table.metric, oracle_result, sbs_result FROM (SELECT scenario_name, fold, approach, metric, result as oracle_result FROM `vbs_sbs` WHERE approach='oracle') as oracle_table JOIN (SELECT scenario_name, fold, approach, metric, result as sbs_result FROM `vbs_sbs` WHERE approach='sbs') as sbs_table ON oracle_table.scenario_name = sbs_table.scenario_name AND oracle_table.fold=sbs_table.fold AND oracle_table.metric = sbs_table.metric) as vbs_sbs JOIN voting_ranking ON vbs_sbs.scenario_name = voting_ranking.scenario_name AND vbs_sbs.fold = voting_ranking.fold AND vbs_sbs.metric = voting_ranking.metric WHERE vbs_sbs.metric='par10') as final WHERE metric='par10' AND approach='voting_ranking_1_2_3_4_6' AND NOT scenario_name='CSP-Minizinc-Obj-2016' GROUP BY approach  ")
    voting_ranknig_weighting_12346 = get_dataframe_for_sql_query(
        "SELECT approach, AVG(n_par10) as result FROM (SELECT vbs_sbs.scenario_name, vbs_sbs.fold, voting_ranking.approach, vbs_sbs.metric, voting_ranking.result, ((voting_ranking.result - vbs_sbs.oracle_result)/(vbs_sbs.sbs_result -vbs_sbs.oracle_result)) as n_par10,vbs_sbs.oracle_result, vbs_sbs.sbs_result FROM (SELECT oracle_table.scenario_name, oracle_table.fold, oracle_table.metric, oracle_result, sbs_result FROM (SELECT scenario_name, fold, approach, metric, result as oracle_result FROM `vbs_sbs` WHERE approach='oracle') as oracle_table JOIN (SELECT scenario_name, fold, approach, metric, result as sbs_result FROM `vbs_sbs` WHERE approach='sbs') as sbs_table ON oracle_table.scenario_name = sbs_table.scenario_name AND oracle_table.fold=sbs_table.fold AND oracle_table.metric = sbs_table.metric) as vbs_sbs JOIN voting_ranking ON vbs_sbs.scenario_name = voting_ranking.scenario_name AND vbs_sbs.fold = voting_ranking.fold AND vbs_sbs.metric = voting_ranking.metric WHERE vbs_sbs.metric='par10') as final WHERE metric='par10' AND approach='voting_ranking_weighting_1_2_3_4_6' AND NOT scenario_name='CSP-Minizinc-Obj-2016' GROUP BY approach  ")

    fig, ax = plt.subplots()  # Create a figure containing a single axes.

    width = 0.18  # the width of the bars
    ax.bar(0.7, voting1234567.result, width, color=color1, label='Majority Voting')
    ax.bar(0.9, voting_ranking_1234567.result, width, color=color2, label='Ranked Voting (avg ranking)')
    ax.bar(1.1, voting_weighting_1234567.result, width, color=color3, label='Weighted Voting')
    ax.bar(1.3, voting_ranknig_weighting_1234567.result, width, color=color4,
           label='Weighted-Ranked Voting (avg ranking)')

    ax.bar(1.7, voting24567.result, width, color=color1)
    ax.bar(1.9, voting_ranking_24567.result, width, color=color2)
    ax.bar(2.1, voting_weighting_24567.result, width, color=color3)
    ax.bar(2.3, voting_ranknig_weighting_24567.result, width, color=color4)

    #ax.bar(2.7, voting12346.result, width, color=color1)
    #ax.bar(2.9, voting_ranking_12346.result, width, color=color2)
    #ax.bar(3.1, voting_weighting_12346.result, width, color=color3)
    #ax.bar(3.3, voting_ranknig_weighting_12346.result, width, color=color4)
    ...
    ax.set_xticks([1, 2])
    ax.set_xticklabels(["f_voting", "s_voting"])

    ax.text(0.7, float(voting1234567.result), round(float(voting1234567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(0.9, float(voting_ranking_1234567.result), round(float(voting_ranking_1234567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(1.1, float(voting_weighting_1234567.result), round(float(voting_weighting_1234567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(1.3, float(voting_ranknig_weighting_1234567.result), round(float(voting_ranknig_weighting_1234567.result), 3), ha='center', va='bottom', rotation=0)

    ax.text(1.7, float(voting24567.result), round(float(voting24567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(1.9, float(voting_ranking_24567.result), round(float(voting_ranking_24567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(2.1, float(voting_weighting_24567.result), round(float(voting_weighting_24567.result), 3), ha='center', va='bottom', rotation=0)
    ax.text(2.3, float(voting_ranknig_weighting_24567.result), round(float(voting_ranknig_weighting_24567.result), 3), ha='center', va='bottom', rotation=0)
