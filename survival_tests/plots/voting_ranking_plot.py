...
def generate_sbs_vbs_change_table():
    ...
    ax.bar(1, voting.result, width, color=color1, label='Voting')
    ax.bar(2, voting_rank.result, width, color=color1, label='Voting Ranking')

    ax.set_xticks([1, 2])
    ax.set_xticklabels(["Voting", "Voting Ranking"])

    #ax.text(1, float(voting.result), round(float(voting.result) - voting_normal, 3), ha='center', va='bottom', rotation=0)
    ax.text(2, float(voting_rank.result), round(float(voting_rank.result) - voting_normal, 3), ha='center', va='bottom', rotation=0)
