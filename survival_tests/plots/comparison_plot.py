...
def generate_sbs_vbs_change_table():
    ...
    ax.text(13.8, np.average(voting_weighting_cross.result) - 0.04, round(np.average(voting_weighting_cross.result), 2), ha='center', va='bottom', rotation=0)
    plt.axhline(np.average(voting_weighting_cross.result), color=voting_color, linestyle='dashed', linewidth=1)

    ind = np.arange(len(stacking.scenario_name))
    width = 0.2  # the width of the bars
    ax.bar(ind, stacking.result, width, color=stacking_color, label='Stacking')
    ax.bar(ind - width, voting_weighting_cross.result, width, color=voting_color, label='Voting with cross validation')
